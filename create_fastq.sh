...
outputdir=$2
reference=$3
#samtoolsenv="/mnt/cg1/cg8/Päivi/software/samtools"
echo $input
...
#		$samtoolsenv/samtools rmdup -s $outputdir/$sample".sorted.map-ont.bam" $outputdir/$sample".sorted.map-ont_rm.bam"
#		grep -A3 -f $f <(samtools fastq $outputdir/$sample".sorted.map-ont_rm.bam")
		grep -A3 -f $f <(samtools fastq $outputdir/$sample".sorted.map-ont_rm.bam") | sed '/^--$/d' >> $outputdir/fastq/$id.fq
		done;
#        for best in $outputdir/reads_in_insertions/*/$id".txt_largest.txt";
#	    do
#	    grep median $best | grep -v -w "." >> $outputdir/fastq/$id"_best_reads.txt"
#		done;
#	filesize=$(wc -l $outputdir/fastq/$id"_best_reads.txt" | cut -f1 -d' ')
#        if [[ $filesize == 0 ]]
#        then
#            continue
#        fi
...
#           half=1
#        fi
#	sequence_name=$(sort -k3,3n $outputdir/fastq/$id"_best_reads.txt" | head -n $half | tail -n 1 | cut -f 2)
	sequence=$(echo $line | cut -f5 -d' ')
	chr=$(echo $line | cut -f1 -d' ')
        start=$(echo $line | cut -f2 -d' ')
#	start=$(sort -k3,3n $outputdir/fastq/$id"_best_reads.txt" | head -n $half | tail -n 1 | cut -f 4)
#	insertion=$(sort -k3,3n $outputdir/fastq/$id"_best_reads.txt" | head -n $half | tail -n 1 | cut -f 3)
	echo $start
...
#        grep -A1 $sequence_name $outputdir/fastq/$id.fq | tail -n1 >> $outputdir/fastq/$id"_1.fasta"
 #       printf ">" > $outputdir/fastq/$id"_read.fasta"
 #       echo $sequence_name >> $outputdir/fastq/$id"_read.fasta"
#	grep -A1 $sequence_name $outputdir/fastq/$id.fq | tail -n1
#        grep -A1 $sequence_name $outputdir/fastq/$id.fq | tail -n1 >> $outputdir/fastq/$id"_read.fasta"
...
#	    start_before=1
#	fi
#	samtools faidx $outputdir/fastq/$id"_read.fasta"
#	samtools faidx $outputdir/fastq/$id"_read.fasta" $sequence_name":"$start_before"-"$start_after | grep -v ">" >> $outputdir/fastq/$id"_1.fasta"
	...
	COUNTER=1
		...
		/mnt/cg8/Päivi/software/racon/build/bin/racon $outputdir/fastq/$id.fq $outputdir/fastq/$id"_"$COUNTER".sam" $outputdir/fastq/$id"_"$COUNTER.fasta | cut -f1 -d' '> $outputdir/fastq/$id"_"$COUNTER"_new".fasta
...
#	bwa mem /mnt/cg38/Paivi/sniffles_data/reads/fastq/$id"_reference.fasta" /mnt/cg38/Paivi/sniffles_data/reads/fastq/$id"_"$COUNTER".fasta" > "/mnt/cg38/Paivi/sniffles_data/reads/fastq/"$id"_aligned.sam"
	bwa mem /mnt/cg8/reference-genomes/GRCh38_no_alt/GRCh38_no_alt.fasta $outputdir/fastq/$id".fasta" >  $outputdir"/fastq/"$id"_aligned.sam"
